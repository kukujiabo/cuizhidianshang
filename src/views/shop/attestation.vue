    ...
    <div class="atterviewbody">
      ...
      <div class="attredbody">
        <div class="atterview">
          ...
          <!-- 个人认证 -->
          <el-form v-show="status===1" ref="form" :model="form" class="grrz">
            <div class="grrz-xm">
              <span>姓名</span>
              <input v-model="form.name" type="text" class="grrz-xm-sr">
            </div>
            <div class="grrz-zjlx">
              <span>证件类型</span>
              <el-input v-model="form.zjlx" placeholder="中国大陆居民身份证" class="grrz-zjlx-srl" :disabled="true" />
            </div>
            <div class="grrz-sfzhm">
              <span>身份证号码</span>
              <input v-model="form.sfz" type="text" class="grrz-sfzhm-srl">
            </div>
            ...
            <div class="grrz-huoquyzm">
              <span>短信验证码:</span>
              <input v-model="form.yzm" type="text" class="grrz-huoquyzmk">
              <el-button v-if="!isDisabled" type="text" @click="dxyzm">{{ content }}</el-button>
              <el-button v-if="isDisabled" type="text">{{ content }}</el-button>
            </div>
            <el-button class="grrz-tijiao-btn" @click="submit(status)">提交认证信息</el-button>
          </el-form>
          <!-- 企业认证 -->
          <el-form v-show="status===2" ref="form2" :model="form2" class="qyrz">
            <div class="qyrz-xm">
              <span>企业名称</span>
              <input v-model="form2.qymc" type="text" class="qyrz-xm-sr">
            </div>
            <div class="qyrz-zjlx">
              <span>法人</span>
              <input v-model="form2.fr" type="text" class="qyrz-zjlx-srl">
            </div>
            <div class="qyrz-sfzhm">
              <span>法人身份证号</span>
              <input v-model="form2.frsfz" type="text" class="qyrz-sfzhm-srl">
            </div>
            <div class="qyrz-fzrsfzhm">
              ...
              <el-upload
                ...
                :limit="1"
                accept=".jpg,.jpeg,.png,.gif,.bmp,.pdf,.JPG,.JPEG,.PBG,.GIF,.BMP,.PDF"
                :file-list="formfileList1"
                :on-change="changeFileqyzm"
              >
            ...
            <div class="qyrz-shdm">
              <span>统一社会信用代码</span>
              <input v-model="form2.shxydm" type="text" class="qyrz-shdm-sr">
            </div>
            <div class="qyrz-jgzj">
              ...
              <el-upload
                ...
                :on-change="changeFileqyyyzz"
                :file-list="formfileList3"
                :auto-upload="false"
              >
            ...
            <div class="qyrz-huoquyzm">
              <span>短信验证码:</span>
              <input v-model="form2.yzm" type="text" class="qyrz-huoquyzmk">
              <el-button v-if="!isDisabled" type="text" @click="dxyzm">{{ content }}</el-button>
              <el-button v-if="isDisabled" type="text">{{ content }}</el-button>
            </div>
          ...
          <!-- 其他机构认证 -->
          <el-form v-show="status===3" ref="form3" :model="form3" class="attr-xx">
            <div class="atterrzxx">
              <span>机构名称</span>
              <input v-model="form3.organizationName " type="text" class="atterjigouname">
            </div>
            <div class="attr-fzr">
              <span>负责人</span>
              <input v-model="form3.userName " type="text" class="atterfzr1">
            </div>
            <div class="attr-fzrsfz">
              <span>负责人身份证号码</span>
              <input v-model="form3.idCard " type="text" class="atterfzrsfz1">
            </div>
            <div class="attr-fzrsfzzp">
              ...
              <el-upload
                ...
                :limit="1"
                accept=".jpg,.jpeg,.png,.gif,.bmp,.pdf,.JPG,.JPEG,.PBG,.GIF,.BMP,.PDF"
                :file-list="form3fileList1"
                :on-change="changeFileqtjgzm"
              >
              ...
              <div style="margin-top: -34px;">
                <el-upload
                  ...
                  :limit="1"
                  accept=".jpg,.jpeg,.png,.gif,.bmp,.pdf,.JPG,.JPEG,.PBG,.GIF,.BMP,.PDF"
                  :file-list="form3fileList2"
                  :on-change="changeFileqtjgfm"
                >
            ...
            <div class="attr-jgzj">
              ...
              <el-upload
                ...
                accept=".jpg,.jpeg,.png,.gif,.bmp,.pdf,.JPG,.JPEG,.PBG,.GIF,.BMP,.PDF"
                :on-remove="handleRemove"
                :on-change="changeFileqtjgyyzz"
                :file-list="form3fileList3"
                :auto-upload="false"
              >
            ...
            <div class="huoquyzm">
              <span>短信验证码:</span>
              <input v-model="form3.verifyCode" type="text" class="huoquyzmk">
              <el-button v-if="!isDisabled" type="text" @click="dxyzm">{{ content }}</el-button>
              <el-button v-if="isDisabled" type="text">{{ content }}</el-button>
            </div>
...
export default {
  ...
  methods: {
    ...
    async submit(status) {
      ...
      if (status == 1) {
        ...
        // }
          ...
          const res = await alterstore(data)
    ...
    async getCertify1() {
      const data = await getCertify(this.form.shopId)
