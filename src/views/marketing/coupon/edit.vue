 ...
 * @LastEditors: lee
 * @LastEditTime: 2020-07-22 00:40:08
 * @Description: 编辑优惠券
-->
<template>
        ...
        <el-card title shadow="never">
          ...
          <div class="inner-form-wrapper">
            ...
            <el-form-item required prop="cType" label="优惠券类型：">
              <el-radio v-model="form.cType" :label="1" disabled>满减优惠券</el-radio>
              <el-radio v-model="form.cType" :label="2" disabled>折扣优惠券</el-radio>
            </el-form-item>
            <el-form-item required prop="isThreshold" label="使用门槛：">
              <div class="radius-item">
                <el-radio v-model="form.isThreshold" :label="false" disabled>无门槛</el-radio>
              </div>
              <div class="radius-item">
                <el-radio v-model="form.isThreshold" :label="true" disabled>有门槛</el-radio>
            ...
            <el-form-item required prop="goodsType" label="适用商品：">
              <div class="radius-item">
                <el-radio v-model="form.goodsType" :label="1" disabled>全部商品</el-radio>
              </div>
              <div class="radius-item">
                <el-radio v-model="form.goodsType" :label="2" disabled>制定商品</el-radio>
                <div v-if="form.goodsType==2" style="display:inline">
                  <span class="input-left">添加商品：</span>
                  <el-button class="btn default" type="default" icon="el-icon-plus" disabled @click="onGoods()">添加指定商品</el-button>
                </div>
              </div>
                    ...
                    <el-table-column align="right" label="操作" width="60">
                      <template slot-scope="scope">
                        <div style="padding-right:0px;">
                          <el-button type="text" disabled @click="delGoods(scope.row)">移除</el-button>
                        </div>
                      </template>
                    </el-table-column>
                  </el-table>
            ...
            <el-form-item required prop="time" label="使用时间：">
              <div class="radius-item">
                <el-radio v-model="form.timeType" class="none" style="margin-right: 10px;" :label="1" disabled>&nbsp;</el-radio>
                <el-date-picker
                  v-model="form.time"
              ...
              <div class="radius-item">
                <el-radio v-model="form.timeType" class="none" style="margin-right: 10px;" :label="2" disabled>&nbsp;</el-radio>
        ...
        <el-card title shadow="never">
          ...
          <div class="inner-form-wrapper">
            <el-form-item required prop="lateDays" label="领取次数限制：">
              <div>
                <el-radio v-model="form.getCountType" :label="0" disabled>不限次数</el-radio>
                <span class="radius-tips">每人最多领取100张</span>
              </div>
              <div>
                <el-radio v-model="form.getCountType" :label="1" disabled>每次限领几次：</el-radio>
                <el-input
            ...
            <el-form-item prop="isSingle" label="活动叠加限制：">
              <el-checkbox v-model="form.isSingle" :true-label="1" :accesskey="0" disabled>优惠仅原价购买时可用，与其他优惠活动互斥</el-checkbox>
            </el-form-item>
...
<script>
import { getCoupon, upCoupon } from '@/api/marketing'
// import { queryDocuments } from '@/api/document'
import { Host } from '@/config'
export default {
  data() {
    var checkIssueQty = (rule, value, callback) => {
      if (this.oldIssueQty > value) {
        return callback(new Error('发行量只能增加不能减少，不得小余' + this.oldIssueQty))
      } else {
        callback()
      }
    }
    return {
      ...
      rules: {
        ...
        issueQty: [
          { validator: checkIssueQty, trigger: 'blur' }
        ]
      }
    }
  },
    ...
    submit() {
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          const res = await upCoupon(this.form)
          if (res.success) {
